...
public class TempleteEJB {
	 ...
	 */
	public void createEJBGeneric(String nameModule, String nameProject)
			throws FileNotFoundException {
		...
		try {
			map.put("pack", pack);
			StringWriter writer = this.util.executeTemplate("ejbGenerico.vm",
					map, "templates");
	 ...
	 * @throws FileNotFoundException
	 */
	public void createEJB(List<Table> tables, String nameModule,
		...
		for (Table table : tables) {
			try {
				map.put("table", table);
				map.put("pack", pack);
				StringWriter writer = this.util.executeTemplate("ejbs.vm", map,
						"templates");
				salidatxt = new PrintStream(this.path + "/" + nameProject + "/"
						+ nameProject + "/" + nameProject + "-" + nameModule
